        ...
        for id1 in range(4):
            for id2 in range(4):
                ...
                contrac_kaon_temp = (contrac_kaon_temp + value_g5Gamma[id1] * value_Gammag5[id2] *
                                     contract("ec,ec",  peram_u[t_sink, :, :, col_g5Gamma[id1], row_Gammag5[id2]], cp.conj(peram_s[t_sink, :, :, row_g5Gamma[id1], col_Gammag5[id2]])))

                contrac_etas_temp = (contrac_etas_temp + value_g5Gamma[id1] * value_Gammag5[id2] *
                                     contract("ec,ec",  peram_s[t_sink, :, :, col_g5Gamma[id1], row_Gammag5[id2]], cp.conj(peram_s[t_sink, :, :, row_g5Gamma[id1], col_Gammag5[id2]])))

        ed = time.time()
        print("compute corr t_sink t_source %d %d done, time used: %.3f s" %
              (t_sink, t_source, ed-st))

        corr_pion[0, (t_sink-t_source+Nt) % Nt] = corr_pion[0,
                                                            (t_sink-t_source+Nt) % Nt] + contrac_pion_temp
        corr_kaon[0, (t_sink-t_source+Nt) % Nt] = corr_kaon[0,
                                                            (t_sink-t_source+Nt) % Nt] + contrac_kaon_temp
        corr_etas[0, (t_sink-t_source+Nt) % Nt] = corr_etas[0,
                                                            (t_sink-t_source+Nt) % Nt] + contrac_etas_temp

    del peram_u, peram_s
    cp._default_memory_pool.free_all_blocks()

    ed0 = time.time()
    print("****************all complete for t_source %d, time used: %.3f s****************" %
          (t_source, ed0-st0))
...
write_data_ascii(corr_pion, Nt, Nx,
                 "/beegfs/home/zhangxin/content/LapH/contraction_code/corr_uu_gamma5_Px0Py0Pz0_conf%s.dat" % (conf_id))
write_data_ascii(corr_kaon, Nt, Nx,
                 "/beegfs/home/zhangxin/content/LapH/contraction_code/corr_us_gamma5_Px0Py0Pz0_conf%s.dat" % (conf_id))
